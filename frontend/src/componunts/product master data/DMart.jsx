...
const DMartData = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await api.get("/dmart/fetch-data", {
      ...
      setError("Failed to fetch DMart data.");
    } finally {
      setLoading(false);
    }
  }, [currentPage, search]);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "DMart_Data");
    XLSX.writeFile(wb, `DMart_Products_Page_${currentPage}.xlsx`);
  };
            ...
            <table className="w-full min-w-max table-auto text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={dmartColumns.length} className="p-20 text-center">
                      <Typography
                        variant="h6"
                        color="blue-gray"
