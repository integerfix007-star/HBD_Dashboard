...
const flipkartColumns = [
  ...
  { key: "brand", label: "Brand", width: "w-1/12" },
  { key: "rating", label: "Rating", width: "w-1/12" },
  { key: "stock_status", label: "Stock", width: "w-1/12" },
];
...
const FlipkartData = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await api.get("/flipkart/fetch-data", {
      ...
      setError("Failed to fetch Flipkart data.");
    } finally {
      setLoading(false);
    }
  }, [currentPage, search]);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Flipkart_Data");
    XLSX.writeFile(wb, `Flipkart_Products_Page_${currentPage}.xlsx`);
  };
            ...
            <table className="w-full min-w-max table-auto text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={flipkartColumns.length} className="p-20 text-center">
                      <Typography
                        variant="h6"
                        color="blue-gray"
