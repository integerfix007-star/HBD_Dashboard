...
const ypColumns = [
  ...
  { key: "pincode", label: "PIN Code", width: "w-1/12" },
...
const YellowPagesData = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get("/yellow-pages/fetch-data", {
      ...
      console.error("Yellow Pages Frontend Error:", err);
    } finally {
      setLoading(false);
    }
  ...
  const handleDownload = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "YellowPages_Data");
    XLSX.writeFile(workbook, "YellowPages_Export.xlsx");
  };
      ...
      <Card className="h-full w-full">
        <CardHeader floated={false} shadow={false} className="rounded-none p-4">
          <div className="mb-4 flex items-center justify-between gap-8">
            <div>
              <Typography variant="h5" color="blue-gray">Yellow Pages Master</Typography>
            ...
            <table className="w-full min-w-max table-auto text-left">
              ...
              <tbody>
                {pageData.length > 0 ? pageData.map((row, index) => (
                  ...
                  <tr>
                    <td colSpan={ypColumns.length} className="p-4 text-center">No records found.</td>
                  </tr>
                )}
              </tbody>
            </table>
