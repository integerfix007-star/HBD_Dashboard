...
const sgColumns = [
  { key: "name", label: "School Name", width: "w-3/12" },
  { key: "category", label: "Category", width: "w-2/12" },
  { key: "subcategory", label: "Subcategory", width: "w-2/12" },
  { key: "city", label: "City", width: "w-2/12" },
  { key: "state", label: "State", width: "w-2/12" },
  { key: "pincode", label: "PIN", width: "w-1/12" },
];
...
const SchoolGISData = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get("/schoolgis/fetch-data", {
      ...
      console.error("SchoolGIS Frontend Error:", err);
    } finally {
      setLoading(false);
    }
  ...
  const handleDownload = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "SchoolGIS_Data");
    XLSX.writeFile(workbook, "SchoolGIS_Export.xlsx");
  };
      ...
      <Card className="h-full w-full">
        <CardHeader floated={false} shadow={false} className="rounded-none p-4">
          <div className="mb-4 flex items-center justify-between gap-8">
            <div>
              <Typography variant="h5" color="blue-gray">School GIS Data Master</Typography>
          ...
          <div className="flex flex-col justify-between gap-4 md:flex-row">
            <div className="w-full md:w-72">
              <Input label="Search School Name" value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} />
            </div>
            <div className="w-full md:w-72">
            ...
            <table className="w-full min-w-max table-auto text-left">
              ...
              <tbody>
                {pageData.length > 0 ? pageData.map((row, index) => (
                  ...
                  <tr>
                    <td colSpan={sgColumns.length} className="p-4 text-center">No records found.</td>
                  </tr>
                )}
              </tbody>
            </table>
