...
const poColumns = [
  { key: "pincode", label: "PIN Code", width: "w-2/12" },
  { key: "area", label: "Area/Branch", width: "w-3/12" },
  { key: "taluka", label: "Taluka", width: "w-2/12" },
  { key: "city", label: "City/District", width: "w-2/12" },
  { key: "state", label: "State", width: "w-2/12" },
];
...
const POIndiaData = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get("/post-office/fetch-data", {
      ...
      console.error("PostOffice Frontend Error:", err);
    } finally {
      setLoading(false);
    }
  ...
  const handleDownload = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "PostOffice_Data");
    XLSX.writeFile(workbook, "India_Post_Office_Export.xlsx");
  };
      ...
      <Card className="h-full w-full">
        <CardHeader floated={false} shadow={false} className="rounded-none p-4">
          <div className="mb-4 flex items-center justify-between gap-8">
            <div>
              <Typography variant="h5" color="blue-gray">India Post Office Master</Typography>
          ...
          <div className="flex flex-col gap-4 md:flex-row">
            <Input label="Search Area Name" value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} />
            <Input label="Filter City/District" value={citySearch} onChange={(e) => { setCitySearch(e.target.value); setCurrentPage(1); }} />
          </div>
        </CardHeader>
                  ...
                  <tr key={index} className="even:bg-blue-gray-50/50">
                    {poColumns.map((col) => (
                      <td key={col.key} className="p-4 border-b border-blue-gray-50">
                        <Typography variant="small" color="blue-gray" className="font-normal">{row[col.key] || "-"}</Typography>
                      </td>
                    ))}
                  </tr>
                )) : (
