...
const GoogleData = () => {
  ...
  const fetchGoogleData = useCallback(async () => {
    ...
    setError(null);
    try {
      // ✅ FIXED: Standardized route matching your other pages
      const response = await api.get("/google-map/fetch-data", {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Google_Data");
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          <Typography variant="h4" className="font-bold text-blue-gray-900">
            Google Listing Master
          </Typography>
          <Typography variant="small" className="font-medium text-gray-500">
            {error ? (
              <span className="text-red-500 font-bold">{error}</span>
            ) : (
              `Displaying verified records from Google (${totalRecords} total)`
            )}
          </Typography>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outlined" size="sm" className="flex items-center gap-2" onClick={fetchGoogleData}>
            ...
            <table className="w-full min-w-[1200px] table-fixed text-left">
              <thead>
                <tr>
                  {googleColumns.map((col) => (
                    <th key={col.key} style={{ width: col.width }} className="border-y border-blue-gray-100 bg-blue-gray-50/50 p-4 transition-colors">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={googleColumns.length} className="p-20 text-center">
