...
const schoolgisColumns = [
  { key: "name", label: "School Name", width: 220 },
...
const SchoolgisData = () => {
  ...
  const fetchSchoolgisData = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const queryParams = new URLSearchParams({
        source: "schoolgis", // Requesting Schoolgis source data from master table
        page: currentPage,
        limit: limit,
        search: search,
        city: citySearch,
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Schoolgis_Data");
    XLSX.writeFile(wb, `Schoolgis_Data_Page_${currentPage}.xlsx`);
  };
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          <Typography variant="h4" className="font-bold text-blue-gray-900">
            Schoolgis Listing Master
          </Typography>
          <Typography variant="small" className="font-medium text-gray-500">
            {error ? (
              <span className="text-red-500 font-bold">{error}</span>
            ) : (
              `Displaying verified records from Schoolgis (${totalRecords} total)`
            )}
          </Typography>
        </div>
            ...
            <table className="w-full min-w-[1200px] table-fixed text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={schoolgisColumns.length} className="p-20 text-center">
