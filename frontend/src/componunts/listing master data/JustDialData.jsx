...
const JustDialData = () => {
  ...
  const fetchJustDialData = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const queryParams = new URLSearchParams({
        source: "just-dial", // Explicitly requesting JustDial source data
        page: currentPage,
        limit: limit,
        search: search,
        city: citySearch,
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "JustDial_Data");
    XLSX.writeFile(wb, `JustDial_Page_${currentPage}.xlsx`);
  };
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          <Typography variant="h4" className="font-bold text-blue-gray-900">
            JustDial Listing Master
          </Typography>
          <Typography variant="small" className="font-medium text-gray-500">
            {error ? (
              <span className="text-red-500 font-bold">{error}</span>
            ) : (
              `Displaying verified records from JustDial (${totalRecords} total)`
            )}
          </Typography>
        </div>
            ...
            <table className="w-full min-w-[1200px] table-fixed text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={justDialColumns.length} className="p-20 text-center">
