...
const AsklailaDataTable = () => {
  ...
  const fetchAsklailaData = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Asklaila_Data");
    XLSX.writeFile(wb, `Asklaila_Page_${currentPage}.xlsx`);
  };
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          <Typography variant="h4" className="font-bold text-blue-gray-900">
            Asklaila Listing Master
          </Typography>
          <Typography variant="small" className="font-medium text-gray-500">
            {error ? (
              <span className="text-red-500 font-bold">{error}</span>
            ) : (
              `Displaying verified records from Asklaila (${totalRecords} total)`
            )}
          </Typography>
        </div>
            ...
            <table className="w-full min-w-[1200px] table-fixed text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={asklailaColumns.length} className="p-20 text-center">
...
export default AsklailaDataTable;