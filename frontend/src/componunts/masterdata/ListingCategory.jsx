...
const categoryColumns = [
  ...
  { key: "name", label: "Category Name", width: 250 },
  { key: "subcategory", label: "Sub-Category", width: 250 },
  { key: "city", label: "City", width: 140 },
  { key: "state", label: "State", width: 140 },
  { key: "area", label: "Area", width: 140 },
  { key: "total_records", label: "Record Count", width: 140 },
];
...
const ListingCategory = () => {
  ...
  const fetchCategoryData = useCallback(async () => {
    ...
    try {
      const queryParams = new URLSearchParams({
        source: "listing-category", // Requesting category slice from master table
        page: currentPage,
        limit: limit,
        search: search,
        city: citySearch,
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Listing_Categories");
    XLSX.writeFile(wb, `Listing_Category_Page_${currentPage}.xlsx`);
  };
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          <Typography variant="h4" className="font-bold text-blue-gray-900">
            Listing Category Master
          </Typography>
          <Typography variant="small" className="font-medium text-gray-500">
            {error ? (
              <span className="text-red-500 font-bold">{error}</span>
            ) : (
              `Displaying categorized business listings (${totalRecords} total)`
            )}
          </Typography>
        </div>
            ...
            <table className="w-full min-w-[1200px] table-fixed text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={categoryColumns.length} className="p-20 text-center">
                      <Typography variant="h6" color="blue-gray" className="opacity-40 italic">
                        {error || "No category records found"}
                      </Typography>
                    </td>
                  </tr>
                )}
