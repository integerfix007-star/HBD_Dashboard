...
const CountryData = () => {
  ...
  const [search, setSearch] = useState(""); // Search within Country
  const [countryFilter, setCountryFilter] = useState(""); // Specific Country filter

  const fetchCountryWiseData = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const queryParams = new URLSearchParams({
        source: "country-master", // Backend keyword for Country-wise master data
        page: currentPage,
        limit: limit,
        search: search,           // General search
        country: countryFilter    // Specific Country filtering
      });
      ...
      setError("Failed to fetch country data from backend");
    } finally {
      setLoading(false);
    }
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Country_Wise_Data");
    XLSX.writeFile(wb, `Country_Data_Page_${currentPage}.xlsx`);
  };
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          <Typography variant="h4" className="font-bold text-blue-gray-900">
            Country Wise Master Data
          </Typography>
          <Typography variant="small" className="font-medium text-gray-500">
            {error ? (
              <span className="text-red-500 font-bold">{error}</span>
            ) : (
              `Displaying verified records grouped by Country (${totalRecords} total)`
            )}
          </Typography>
        </div>
          ...
          {loading ? (
            <div className="flex flex-col justify-center py-24 items-center gap-4">
              ...
              <Typography className="animate-pulse font-medium text-gray-600">
                Retrieving International Records...
              </Typography>
            </div>
          ) : (
            <table className="w-full min-w-[1200px] table-fixed text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={countryColumns.length} className="p-20 text-center">
                      <Typography variant="h6" color="blue-gray" className="opacity-40 italic">
                        {error || "No country-specific records found"}
                      </Typography>
                    </td>
                  </tr>
                )}
