...
const cityColumns = [
  { key: "city", label: "City Name", width: 160 },
...
const CityData = () => {
  ...
  const [search, setSearch] = useState(""); // Searches specific City
  const [stateSearch, setStateSearch] = useState(""); // Filter by State

  const fetchCityWiseData = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const queryParams = new URLSearchParams({
        source: "city-master", // Backend keyword for City-wise master data
        page: currentPage,
        limit: limit,
        city: search,         // Filter by City Name
        state: stateSearch    // Filter by State Name
      });
      ...
      setError("Failed to fetch city data from backend");
    } finally {
      setLoading(false);
    }
  }, [currentPage, search, stateSearch]);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "City_Wise_Data");
    XLSX.writeFile(wb, `City_Data_Page_${currentPage}.xlsx`);
  };
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          <Typography variant="h4" className="font-bold text-blue-gray-900">
            City Wise Master Data
          </Typography>
          <Typography variant="small" className="font-medium text-gray-500">
            {error ? (
              <span className="text-red-500 font-bold">{error}</span>
            ) : (
              `Displaying verified records grouped by City (${totalRecords} total)`
            )}
          </Typography>
        </div>
            ...
            <table className="w-full min-w-[1200px] table-fixed text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={cityColumns.length} className="p-20 text-center">
                      <Typography variant="h6" color="blue-gray" className="opacity-40 italic">
                        {error || "No records found for this city/state"}
                      </Typography>
                    </td>
                  </tr>
                )}
