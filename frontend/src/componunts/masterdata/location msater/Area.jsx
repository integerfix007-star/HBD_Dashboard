...
const areaColumns = [
  { key: "area", label: "Area Name", width: 200 },
  ...
  { key: "address", label: "Full Address", width: 320 },
...
const AreaData = () => {
  ...
  const [search, setSearch] = useState(""); // Searches specific Area name
  const [citySearch, setCitySearch] = useState(""); // Filter areas by City
  ...
  const fetchAreaData = useCallback(async () => {
    ...
    try {
      const queryParams = new URLSearchParams({
        source: "area-master", // Backend keyword to trigger Area-wise master data logic
        page: currentPage,
        limit: limit,
        area: search,         // Specific Area filter
        city: citySearch      // Parent City filter
      });
      ...
      setError("Failed to fetch area data from backend");
    } finally {
      setLoading(false);
    }
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Area_Wise_Data");
    XLSX.writeFile(wb, `Area_Data_Page_${currentPage}.xlsx`);
  };
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          <Typography variant="h4" className="font-bold text-blue-gray-900">
            Area Wise Master Data
          </Typography>
          <Typography variant="small" className="font-medium text-gray-500">
            {error ? (
              <span className="text-red-500 font-bold">{error}</span>
            ) : (
              `Displaying verified records grouped by Area (${totalRecords} total)`
            )}
          </Typography>
        </div>
            ...
            <table className="w-full min-w-[1200px] table-fixed text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={areaColumns.length} className="p-20 text-center">
                      <Typography variant="h6" color="blue-gray" className="opacity-40 italic">
                        {error || "No area records found"}
                      </Typography>
                    </td>
                  </tr>
                )}
