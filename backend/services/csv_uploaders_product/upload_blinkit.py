...
def upload_blinkit_data(file_paths):
    if not file_paths:
        print("logic error")
    ...
    try:
        drop_non_essential_indexes(cursor,'blinkit',['category','brand','rating'])
        connection.commit()
        ...
        if upload_success:
            create_non_essential_indexes(cursor,'blinkit',['category','brand','rating'])
            connection.commit()
        cursor.close()
        connection.close()
