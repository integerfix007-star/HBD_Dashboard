...
@bigbasket_api_bp.route('/scrape', methods=['POST'])
...
@bigbasket_api_bp.route('/fetch-data', methods=['GET']) 
def get_bigbasket_data():
    try:
        ...
        products = query.order_by(BigBasketProduct.id.desc()).offset((page - 1) * limit).limit(limit).all()
